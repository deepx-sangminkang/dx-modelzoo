...
This creates an isolated venv at `~/deepx/venv-dx-modelzoo` (override with `DX_INSTALL_DIR=<dir>`) and installs the latest release into it (pin one with `DX_VERSION=vX.Y.Z`). Packages are installed with [uv](https://docs.astral.sh/uv/), which is considerably faster than pip on a dependency tree this size; uv is used if already present and otherwise fetched as a pinned standalone binary into `~/.local/bin`. Set `DX_NO_UV=1` to use pip instead — both install the same packages into the same venv. It defaults to the `cpu` extra — the base package ships **no ONNX Runtime backend at all**, so a plain install without `cpu` or `gpu` can be imported but cannot run inference. Set `DX_EXTRA=gpu` to install the CUDA (`onnxruntime-gpu`) backend instead. The dependency tree (torch, transformers, etc.) is large — expect a ~5 GB download and disk footprint, so don't kill the install partway through on a slow connection.
